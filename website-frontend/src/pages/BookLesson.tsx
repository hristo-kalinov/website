    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        <div className="bg-indigo-700 px-8 py-6">
          ...
          <p className="mt-1 text-indigo-100">Select your preferred time slot</p>
        </div>
        ...
        <div className="p-8">
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-8">
              <h2 className="text-xl font-semibold text-gray-800 mb-6">Available Days</h2>
              ...
              {Object.entries(groupedAvailability).map(([day, blocks]) => (
                <div key={day} className="mb-4 border border-gray-200 rounded-lg overflow-hidden">
                  ...
                  >
                    <h3 className="text-lg font-medium text-gray-800">{dayNames[day]}</h3>
                    <svg
                  ...
                  {expandedDay === parseInt(day) && (
                    <div className="p-4 bg-white">
                      {blocks.map((block, blockIndex) => (
                        <div key={blockIndex} className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                          <div className="flex items-center justify-between mb-4">
                            <span className="font-medium text-gray-800">
                              Available: {block.startTime} - {block.endTime}
                            </span>
                          </div>
                            ...
                            {Array.from({ length: block.endSlot - block.startSlot + 1 }).map((_, i) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`py-2 px-1 text-xs sm:text-sm rounded-md transition-all ${
                          ...
                          {selectedStart?.day === parseInt(day) && selectedStart?.slot >= block.startSlot && selectedStart?.slot <= block.endSlot && (
                            <div>
                              ...
                              <p className="mt-2 text-sm text-gray-500">
                                Selected: {slotToTime(selectedStart.slot)} - {slotToTime(selectedStart.slot + selectedDuration)}
                              </p>
                            </div>
                          )}
                        </div>
            ...
            {/* Schedule Type and Submit Button (same as before) */}
            <div className="mb-8">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Schedule Type</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <label className={`p-4 rounded-lg border-2 cursor-pointer transition-colors ${
                  ...
                  <div className="flex items-center">
                    ...
                    />
                    <div className="ml-3">
                      <span className="block text-sm font-medium text-gray-900">One-time Lesson</span>
                      <span className="block text-xs text-gray-500 mt-1">Single session booking</span>
                    </div>
                  </div>
                </label>
                <label className={`p-4 rounded-lg border-2 cursor-pointer transition-colors ${
                  ...
                  <div className="flex items-center">
                    ...
                    />
                    <div className="ml-3">
                      <span className="block text-sm font-medium text-gray-900">Weekly Lessons</span>
                      <span className="block text-xs text-gray-500 mt-1">Recurring weekly booking</span>
                    </div>
                  </div>
                </label>
              </div>
            ...
            <button
              ...
              className="w-full py-3 px-6 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-70 disabled:cursor-not-allowed shadow-md"
            >
              {selectedStart ? (
                `Book ${frequency === 'weekly' ? 'Weekly' : 'One-time'} Lesson from ${slotToTime(selectedStart.slot)} for ${selectedDuration * 30} minutes`
              ) : (
